...
#define API_ENTPOINT_URL "http://soarhub.com:8000"
...
JETProvider::Glue::OnTimer(const NMEAInfo &basic, const DerivedInfo &calculated) {
  ...
  if (!clock.CheckUpdate(settings.radar.interval))
    return;
...
JETProvider::Glue::CoTick(const NMEAInfo &basic) noexcept
{
  ...
  if (!RadarParser::ParseRadarBuffer(basic, response.body.c_str(), radar) || !radar.validity.IsValid()) {
    //handler->OnJETTraffic(std::vector<std::unique_ptr<JETProvider::Data::Traffic>>(), false);
  } else {
