...
#else
bool CDFDecoder::Decode() {
  LogFormat("CDFDecoder Android Decode");
  NcFile data_file(path.c_str(), NcFile::FileMode::ReadOnly);
  
  if(!data_file.is_valid()) {    
    LogFormat("Decode Error");
    return DecodeError();
  }
  
  LogFormat("Data file with path %s has dims %d vars %d", path.c_str(), data_file.num_dims(), data_file.num_vars());
  ...
  if(!data_file.get_var("lat")->get(lat_vals, lat_size))
    LogFormat("Get Var failed LAT");
  if(!data_file.get_var("lon")->get(lon_vals, lon_size))
    LogFormat("Get Var failed LON");
  LogFormat("Lat vals and lon vals written");
  double lat_min = lat_vals[lat_size - 1];
  double lat_max = lat_vals[0];
  double lon_min = lon_vals[0];
  double lon_max = lon_vals[lon_size - 1];
